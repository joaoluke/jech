...
O **parser** é a segunda etapa do pipeline de compilação. Ele pega o fluxo de tokens do tokenizador e valida que eles formam **padrões de sintaxe** válidos.

Pense nisso como verificar gramática: o tokenizador identificou as palavras, agora o parser verifica se elas formam frases válidas.

### O que o Parser Faz

1. **Valida sintaxe** — garante que tokens apareçam na ordem correta
2. **Cria nós da AST** — constrói uma estrutura em árvore representando o programa
3. **Reporta erros** — diz exatamente o que está errado e onde
...
### Padrão 1: `say(...)` — Declaração de Saída
...
### Padrão 4: `when` — Declaração Condicional
...
### Padrão 5: `else` — Bloco Alternativo
...
Faz o parsing de uma declaração `say(...)`. Valida a sequência de tokens e cria um nó AST do tipo `JECH_AST_SAY`.
...
Faz o parsing de uma declaração `keep x = valor;`. Cria um nó AST com:
...
Faz o parsing de declarações condicionais. Esta é a função de parser mais complexa porque lida com:
...
- `left` → nó da condição
- `right` → bloco então (o que executar se verdadeiro)
- `else_branch` → bloco else (o que executar se falso)
...
Faz o parsing de reatribuição de variável: `x = novoValor;`
...
Tokens de entrada (do tokenizador):
```text
TOKEN_KEEP "idade"
...
report_syntax_error("Condição inválida na declaração 'when'", token.line, token.column);
```
...
1. **Descida recursiva** — cada regra gramatical se torna uma função
